...
int dragon_draw_raw(uint64_t start, uint64_t end, char* dragon, int width, int height,
                    limits_t limits, char id) {
  if (end < start) {
    std::printf("error: start=%ld > end=%ld\n", start, end);
    return -1;
  }
  ...
  for (n = start + 1; n <= end; n++) {
    ...
    if (index < 0 || index > area) {
      std::printf("index is out of range\n");
      return -1;
    }
    dragon[index] = id;
...
void init_canvas(uint64_t start, uint64_t end, char* canvas, char value) {
...
void dump_canvas_rgb(struct rgb* canvas, int width, int height) {
  ...
  for (i = 0; i < width; i++) {
    for (j = 0; j < height; j++) {
      ...
      std::printf("%d %d %d ", pix->r, pix->g, pix->b);
    }
    std::printf("\n");
  }
}
...
int dragon_draw_serial(char** canvas, struct rgb* image, int width, int height, uint64_t size,
                       int nb_colors) {
  int ret = 0;
  char* dragon = NULL;
...
int write_img(struct rgb* image, const char* file, int width, int height) {
  FILE* f = NULL;
...
void dump_limits(limits_t* limits) {
  ...
  std::printf("(%ld,%ld; %ld,%ld)\n", limits->minimums.x, limits->minimums.y, limits->maximums.x,
              limits->maximums.y);
}

int cmp_limits(limits_t* l1, limits_t* l2) {
  if (l1 == NULL || l2 == NULL) return -1;
  return !(l1->maximums.x == l2->maximums.x && l1->maximums.y == l2->maximums.y
           && l1->minimums.x == l2->minimums.x && l1->minimums.y == l2->minimums.y);
}
  ...
  for (i = 0; i < height; i++) {
    for (j = 0; j < width; j++) {
      ...
      if (exp[index] != act[index]) {
        if (verbose)
          std::printf("pix error (%5d, %5d) expected=%2d actual=%2d\n", j, i, exp[index],
                      act[index]);
        sum += 1;
      }
    }
...
int dragon_limits_serial(limits_t* lim, uint64_t nbIterations,
                         __attribute__((unused)) int nb_thread) {
  piece_t piece;
  piece_init(&piece);
  uint64_t start = 0;
...
void piece_merge(piece_t* m1, piece_t* m2) {
  ...
  // Rotate piece #2
  while (orientation.x != m1->orientation.x || orientation.y != m1->orientation.y) {
